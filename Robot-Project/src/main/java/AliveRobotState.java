...
boolean validMove = true;
	...
	public boolean fire(int x, int y) throws InterruptedException
	{
		...
		if(myRobot.getX()!=x || myRobot.getY()!= y )
		{
		 SwingUtilities.invokeLater(() ->
        	{
		robotControl.getArena().fireLazer(myRobot.getX(), myRobot.getY(), x, y);
		});
		}
		for(RobotInfo robot : robotControl.getAllRobots())
		{
			//Dont need to keep looping if you have already shot
			if(valid==true)
			{
				break;
			}
		...
		if(valid == false)
		{
			robotControl.logMessage(myRobot.getName() + " has missed their shot!");
		}
		return valid;
	}
